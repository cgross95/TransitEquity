...
parser.add_argument('--seg', type=str, default='S000',
                    help='Segment of the workforce; S000: Total number of jobs, SE01: Number of jobs with earnings $1250/month or less')
parser.add_argument('--recompute_current_transit_time',
                    action=argparse.BooleanOptionalAction)
parser.add_argument('--resimulate_red_line',
                    action=argparse.BooleanOptionalAction)
parser.add_argument('--speed', type=float, default=20,
                    help='Speed of the Assumed Red Line; default is 20 mph')
parser.add_argument('--period', type=float, default=8,
                    help='Service frequency; Default is 8 minutes')
parser.add_argument('--svi2020', action='store_true',
                    help='Use 2020 census data (otherwise 2018)')
parser.add_argument('--thresh_min', type=int, default=15,
                    help='Lowest commute threshold to consider')
parser.add_argument('--thresh_max', type=int, default=90,
...
# Get transit data
if recompute_current_transit_time is True:
    ...
    get_all_pair_transit_time("gtfs_current.csv", job_flow,
                              tract_file="2020_Gaz_tracts_24.txt",
                              date='11/01/2022', time='08:00AM')

if resimulate_red_line is True:
    ...
    # And wait till "Grizzly server running." shows up
    get_all_pair_transit_time(f"gtfs_red_line_speed_{int(speed)}_period_{int(period)}.csv",
                              job_flow,
                              tract_file="2020_Gaz_tracts_24.txt",
                              date='11/01/2022',
                              time='08:00AM')
...
transit_time = compute_transit_time("gtfs_current.csv",
                                    f"gtfs_red_line_speed_{int(speed)}_period_{int(period)}.csv",
